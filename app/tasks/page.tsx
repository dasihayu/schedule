  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ margin: "0 0 8px", fontSize: "0.95rem", fontWeight: 800, color: "var(--text)", letterSpacing: "-0.01em", fontFamily: "var(--font-display)" }}>
          {title}
        </h2>
        <p style={{ margin: "0 0 20px", fontSize: "0.83rem", color: "var(--text-muted)", lineHeight: 1.5 }}>
          {message}
        </p>
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          <button onClick={onCancel} disabled={loading} style={{ padding: "7px 14px", borderRadius: "var(--radius)", border: "1px solid var(--border)", background: "transparent", color: "var(--text-muted)", fontWeight: 600, fontSize: "0.82rem", cursor: "pointer", fontFamily: "var(--font-display)" }}>
            Batal
          </button>
          <button onClick={onConfirm} disabled={loading} style={{ padding: "7px 14px", borderRadius: "var(--radius)", border: "none", background: "var(--danger)", color: "#fff", fontWeight: 700, fontSize: "0.82rem", cursor: loading ? "default" : "pointer", fontFamily: "var(--font-display)", opacity: loading ? 0.7 : 1, display: "flex", alignItems: "center", gap: 6 }}>
...
export default function TasksPage() {
  ...
  useEffect(() => { loadProjects(); }, [status]); // eslint-disable-line
  ...
  const handleAddTask = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/tasks", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: newTaskTitle.trim(), projectId: selectedProjectId }) });
      const data = await res.json();
      if (!res.ok || !data.task) { setToast(`Error: ${data.error ?? "Failed"}`); return; }
  ...
  const handleToggle = async (task: Task) => {
    ...
    await fetch("/api/tasks", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: task.id, status: next }) });
  ...
  const handleDeleteProject = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch("/api/projects", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: confirmDelete.id }) });
      if (!res.ok) { const data = await res.json(); setToast(`Error: ${data.error ?? "Failed"}`); return; }
  ...
  return (
    <div data-theme={theme} suppressHydrationWarning>
      ...
      <div style={{ maxWidth: 700, margin: "0 auto", padding: "36px 20px 80px" }}>
        ...
        <header style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <a href="/" style={{
              ...
              fontSize: "0.76rem", color: "var(--text-subtle)", textDecoration: "none",
              padding: "5px 11px", borderRadius: "var(--radius)", border: "1px solid var(--border)",
              background: "var(--surface-2)", fontWeight: 600, fontFamily: "var(--font-display)",
              letterSpacing: "0.02em", transition: "color 0.12s, border-color 0.12s",
            }}
            ...
            <div>
              <h1 style={{ fontSize: "1.2rem", fontWeight: 800, letterSpacing: "-0.03em", margin: 0, marginBottom: 1, color: "var(--text)", fontFamily: "var(--font-display)", lineHeight: 1.2 }}>
                Daily Tasks
              </h1>
              <p style={{ fontSize: "0.74rem", color: "var(--text-subtle)", margin: 0, fontWeight: 400 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            {session?.user?.name && (
              <span style={{ fontSize: "0.75rem", color: "var(--text-muted)", padding: "5px 10px", background: "var(--surface-2)", borderRadius: "var(--radius)", border: "1px solid var(--border)", fontFamily: "var(--font-display)", fontWeight: 500 }}>
                {session.user.name}
              </span>
            )}
            ...
            <button onClick={() => signOut({ callbackUrl: "/login" })} style={{ fontSize: "0.75rem", color: "var(--danger)", background: "var(--danger-soft)", border: "1px solid rgba(191,32,32,0.2)", borderRadius: "var(--radius)", padding: "6px 12px", cursor: "pointer", fontWeight: 600, fontFamily: "var(--font-display)" }}>
              Sign Out
            </button>
          </div>
        </header>
        ...
        {/* ── Project Tabs ──────────────────────────────── */}
        <section style={{ marginBottom: 18, animation: "fadeInUp 0.35s ease 0.05s both" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, overflowX: "auto", flexWrap: "nowrap", paddingBottom: 4, scrollbarWidth: "none" }}>
            {projects.map((p) => (
              <div key={p.id} style={{ position: "relative", display: "inline-flex", alignItems: "center", flexShrink: 0 }}>
                <button
                ...
                {selectedProjectId === p.id && (
                  <button onClick={(e) => { e.stopPropagation(); setConfirmDelete(p); }} title="Hapus project"
                    style={{ position: "absolute", right: 7, width: 14, height: 14, borderRadius: "50%", border: "none", background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--danger)", fontSize: "0.65rem", fontWeight: 900, padding: 0, lineHeight: 1 }}>
                    ✕
                  </button>
                )}
              </div>
            ...
            {!showNewProject ? (
              <button className="proj-tab" onClick={() => setShowNewProject(true)} style={{ borderStyle: "dashed", flexShrink: 0 }}>
                + New
              </button>
            ) : (
        ...
        {/* ── Main Task Card ────────────────────────────── */}
        {selectedProjectId ? (
          <div className="app-card" style={{ padding: 0, overflow: "hidden", animation: "fadeInUp 0.35s ease 0.1s both" }}>
            {/* Card header */}
            ...
            {/* Add task */}
            <form onSubmit={handleAddTask} style={{ padding: "11px 18px", borderBottom: "1px solid var(--border-soft)", display: "flex", gap: 8 }}>
              <input
                ref={taskInputRef}
                value={newTaskTitle}
                onChange={(e) => setNewTaskTitle(e.target.value)}
                placeholder="Add a new task…"
                style={{ flex: 1, padding: "7px 11px", borderRadius: "var(--radius)", border: "1px solid var(--border)", background: "var(--surface-2)", color: "var(--text)", outline: "none", fontSize: "0.875rem", fontFamily: "var(--font-body)", fontWeight: 400, transition: "border-color 0.12s" }}
                onFocus={(e) => (e.target.style.borderColor = "var(--primary)")}
                onBlur={(e) => (e.target.style.borderColor = "var(--border)")}
              />
              <button type="submit" disabled={!newTaskTitle.trim() || loading} style={{ padding: "7px 15px", borderRadius: "var(--radius)", border: "none", background: newTaskTitle.trim() ? "var(--primary)" : "var(--surface-3)", color: newTaskTitle.trim() ? "#fff" : "var(--text-subtle)", fontWeight: 700, fontSize: "0.82rem", cursor: newTaskTitle.trim() ? "pointer" : "default", fontFamily: "var(--font-display)", transition: "background 0.12s" }}>
                {loading ? <Spinner /> : "Add"}
              </button>
            </form>
            ...
            {tasksLoading ? (
              ...
              <div style={{ padding: "44px 20px", textAlign: "center", color: "var(--text-subtle)", fontSize: "0.85rem" }}>
                <div style={{ fontSize: "2.2rem", marginBottom: 10, opacity: 0.3 }}>✅</div>
                <p style={{ margin: 0, fontFamily: "var(--font-display)", fontWeight: 500 }}>No tasks yet. Add one above!</p>
              </div>
            ) : (
              <div>
                {pendingTasks.length > 0 && (
                  <div>
                    <div style={{ padding: "8px 18px 4px", fontSize: "0.64rem", fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--warning)", display: "flex", alignItems: "center", gap: 6, fontFamily: "var(--font-display)" }}>
                      <span style={{ width: 5, height: 5, borderRadius: "50%", background: "var(--warning)", display: "inline-block" }} />
                      Pending · {pendingTasks.length}
                    </div>
                    {pendingTasks.map((task, i) => <TaskRow key={task.id} task={task} index={i} onToggle={handleToggle} onDelete={handleDelete} />)}
                  </div>
                )}
                {doneTasks.length > 0 && (
                  <div>
                    <div style={{ padding: "8px 18px 4px", fontSize: "0.64rem", fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--success)", display: "flex", alignItems: "center", gap: 6, borderTop: pendingTasks.length > 0 ? "1px solid var(--border-soft)" : undefined, marginTop: pendingTasks.length > 0 ? 4 : 0, fontFamily: "var(--font-display)" }}>
                      <span style={{ width: 5, height: 5, borderRadius: "50%", background: "var(--success)", display: "inline-block" }} />
                      Done · {doneTasks.length}
                    </div>
                    {doneTasks.map((task, i) => <TaskRow key={task.id} task={task} index={i} onToggle={handleToggle} onDelete={handleDelete} />)}
                  </div>
                )}
              </div>
            )}
            ...
            {tasks.length > 0 && (
              <div style={{ padding: "9px 18px", borderTop: "1px solid var(--border-soft)", fontSize: "0.68rem", color: "var(--text-subtle)", textAlign: "center", fontStyle: "italic" }}>
                Done tasks are hidden 24 hours after creation
              </div>
            )}
          </div>
        ) : (
          <div className="app-card" style={{ padding: "56px 20px", textAlign: "center", color: "var(--text-subtle)", animation: "fadeInUp 0.35s ease" }}>
            <div style={{ fontSize: "2.8rem", marginBottom: 12, opacity: 0.25 }}>📁</div>
            <p style={{ fontWeight: 700, marginBottom: 4, fontFamily: "var(--font-display)", fontSize: "0.9rem" }}>No project selected</p>
            <p style={{ fontSize: "0.78rem", margin: 0 }}>Create or select a project above to get started</p>
          </div>
        )}
      </div>
...
// ── Task Row ──────────────────────────────────────────────────
function TaskRow({ task, index, onToggle, onDelete }: { task: Task; index: number; onToggle: (t: Task) => void; onDelete: (id: string) => void; }) {
  ...
  return (
    <div className="task-row" style={{
      ...
      {/* Title */}
      <span style={{
        flex: 1, fontSize: "0.875rem", fontWeight: 400, lineHeight: 1.4, color: "var(--text)",
        textDecoration: isDone ? "line-through" : "none",
        textDecorationColor: "var(--text-subtle)",
        fontFamily: "var(--font-body)",
      }}>
        <span style={{ fontSize: "0.68rem", fontWeight: 700, color: "var(--text-subtle)", marginRight: 6, fontFamily: "var(--font-mono)", fontVariantNumeric: "tabular-nums" }}>
          {index + 1}.
        </span>
        {task.title}
      </span>
      ...
      {/* Delete */}
      <button className="task-del" onClick={() => onDelete(task.id)} title="Delete task" style={{
        opacity: 0,
        display: "flex", alignItems: "center", justifyContent: "center",
        width: 26, height: 26, borderRadius: "var(--radius)", border: "none",
        background: "transparent", cursor: "pointer", color: "var(--text-subtle)",
        transition: "background 0.12s, color 0.12s",
      }}
        onMouseOver={(e) => { e.currentTarget.style.background = "var(--surface-3)"; e.currentTarget.style.color = "var(--danger)"; }}
        onMouseOut={(e) => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "var(--text-subtle)"; }}
      >
